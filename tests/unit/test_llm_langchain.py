...
from tests.mocks.mock_llm_handler import create_mock_llm_handler
...
def test_configure_chain_with_valid_prompts(resources_dir):
    ...
    prompts_file_path = resources_dir / "prompts.json"
    mock_llm_handler = create_mock_llm_handler()
    with patch("det.llm.llm_langchain.LLMHandler", return_value=mock_llm_handler):
        client = LangChainClient(prompts_file_path=prompts_file_path)

    input_variables = {"risk_statement": "Sample risk statement"}
    client.configure_chain(
        prompt_group="RiskDefinition", input_variables=input_variables
    )

    assert client.chain is not None, "Chain should be configured."
    assert (
        client.input_variables == input_variables
    ), "Input variables should be set correctly."
...
def test_configure_chain_missing_format_instructions(resources_dir):
    ...
    with patch("det.llm.llm_langchain.LLMHandler", return_value=mock_llm_handler):
        ...
        with patch.object(
            ...
            assert (
                client.chain is not None
            ), "Chain should be configured even without format instructions."
...
def test_configure_chain_missing_system_prompt(resources_dir):
    ...
    with patch("det.llm.llm_langchain.LLMHandler", return_value=mock_llm_handler):
        ...
        with patch.object(
            PromptManager, "get_prompts", return_value={"prompt": "User prompt"}
        ):
            with pytest.raises(
                ValueError, match="System prompt for 'RiskDefinition' not found."
            ):
                client.configure_chain(
                    prompt_group="RiskDefinition", input_variables={}
                )
...
def test_generate_response_success(resources_dir):
    ...
    with patch("det.llm.llm_langchain.LLMHandler", return_value=mock_llm_handler):
        ...
        ), "The response should match the mocked return value."
...
def test_generate_response_unconfigured_chain():
    ...
    with patch("det.llm.llm_langchain.LLMHandler", return_value=mock_llm_handler):
        ...
        with pytest.raises(
            ValueError,
            match="The Langchain client has not been configured with a chain or input variables.",
        ):
...
def test_generate_response_retry_mechanism(resources_dir):
    ...
    with patch("det.llm.llm_langchain.LLMHandler", return_value=mock_llm_handler):
        ...
        with pytest.raises(
            ResponseGenerationError, match="Failed after 3 attempts: Parsing error"
        ):
            client.generate_response()
...
def test_configure_chain_missing_user_prompt(resources_dir):
    ...
    with patch("det.llm.llm_langchain.LLMHandler", return_value=mock_llm_handler):
        ...
        with patch.object(
            ...
            with pytest.raises(
                ValueError, match="User prompt for 'RiskDefinition' not found."
            ):
                client.configure_chain(
                    prompt_group="RiskDefinition", input_variables={}
                )
